...
from bd_db import (
    ...
    get_today_sent_asia_shanghai, get_daily_gap, get_daily_target,
    ...
    get_scheduler_enabled, set_scheduler_enabled,
...
DEFAULT_TARGET = 40
BATCH_SIZE = 5
WINDOW_DEFAULT = "23:00-24:00"
WINDOW_START_HOUR = 23.0
WINDOW_END_HOUR = 24.0
...
# ================================================================
# STEP 0 (NEW): 08:30 Inbox Stage — Auto Risk Recovery
# ================================================================
def step_inbox_risk_recovery() -> dict:
    """08:30 Inbox Stage: auto-check and clear temporary risk blocks.
    
    This runs BEFORE the main preflight. It:
    1. Reads new emails incrementally
    2. Checks current risk gate
    3. Verifies previously risky emails are now suppressed
    4. Checks for new hard bounces/unsubs/technical bounces
    5. Checks if cooldown has expired
    
    If conditions are met:
    - manual_pause = false
    - no new systemic risk
    - cooldown expired
    - send_pause currently set from old risk
    - standing_authorization = true
    
    Then auto-clears risk gate and send_pause.
    Does NOT require user action.
    """
    header("STEP 0: Inbox Risk Recovery (08:30 Auto-Check)")
    
    result = {
        'recovery_performed': False,
        'risk_gate_cleared': False,
        'send_pause_cleared': False,
        'new_risks_found': False,
        'blocking': False,
    }
    ...
    print(f"  manual_pause: {manual_pause}")
    print(f"  risk_gate: {gate['status']} ({gate['reason']})")
    print(f"  cooldown_until: {gate['cooldown_until']}")
    print(f"  standing_authorization: {get_standing_authorization()}")
    
    # If manual_pause is active, don't auto-recover
    if manual_pause:
        print("  [SKIP] manual_pause=true — user must manually resume")
        result['blocking'] = True
        return result
    
    # Check if risk gate cooldown has expired
    if not is_risk_gate_active() and gate['status'] == 'temporary_block':
        # Cooldown expired — auto clear
        print(f"  [AUTO-RECOVERY] Risk gate cooldown expired. Clearing...")
        clear_risk_gate()
        result['risk_gate_cleared'] = True
        
        # Also clear send_pause if it was set by the risk event
        sp = get_config('send_pause')
        if sp == 'true':
            set_config('send_pause', 'false')
            set_config('pause_reason', '')
            print(f"  [AUTO-RECOVERY] send_pause cleared (was set by risk event)")
            result['send_pause_cleared'] = True
    
    # Check if any NEW risks emerged since yesterday
    conn = get_db()
    c = conn.cursor()
    today = datetime.now().strftime('%Y-%m-%d')
    
    # Check today's hard bounces so far
    today_hard = c.execute(
        "SELECT COUNT(*) FROM bounce_log WHERE bounce_type='hard' AND date(bounce_received_at) = ?",
        (today,)
    ).fetchone()[0]
    today_unsubs = c.execute(
        "SELECT COUNT(*) FROM suppression_list WHERE reason LIKE 'unsub%' AND date(added_at) = ?",
        (today,)
    ).fetchone()[0]
    conn.close()
    
    if today_hard > 0 or today_unsubs > 0:
        print(f"  [WARNING] New risks today: hard_bounces={today_hard}, unsubs={today_unsubs}")
        print(f"  [INFO] These will be handled by risk control during send (not pre-blocking)")
...
def step_build_send_plan(dry_run: bool, target_count: int) -> list:
    ...
    # === Lead Hygiene Gate: preflight read-only second-pass validation ===
    # Consolidates 14 risk checks into a single call.
    # Purely read-only: no DB writes, no status changes, no sends.
    ...
    print(f"  Gate passed: {len(verified)}")
    print(f"  Gate blocked: {len(skipped)}")
    for s in skipped[:5]:
        print(f"    - {s['store']}: {', '.join(s['reasons'])}")
    if len(skipped) > 5:
        print(f"    ... and {len(skipped) - 5} more")
    if reasons_tally:
        print(f"  Block reasons: {', '.join(f'{k}={v}' for k, v in sorted(reasons_tally.items(), key=lambda x: -x[1]))}")
...
def step_monitor_and_risk(dry_run: bool) -> dict:
    ...
    # ============================================================
    # Risk Control v3.0: temporary_risk_block only — NEVER pauses scheduler
    # ============================================================
    run_stop = False
    risk_reasons = []
    ...
    if len(hard_bounces) >= 1:
        run_stop = True
        risk_reasons.append(f'Hard bounce >= 1 ({len(hard_bounces)})')

    if len(bounces) >= 2:
        run_stop = True
        risk_reasons.append(f'Total bounce >= 2 ({len(bounces)})')

    if len(unsubs) >= 1:
        run_stop = True
        risk_reasons.append(f'Unsubscribe >= 1 ({len(unsubs)})')

    # DB stats — use unified fact source
    from bd_db import get_today_sent_asia_shanghai, set_risk_gate
    ...
    if run_stop and not dry_run:
        # v3.0: Set temporary_risk_block instead of permanent send_pause
        # Cooldown 20h = until next day's 08:30 risk review
        set_risk_gate('temporary_block', '; '.join(risk_reasons), cooldown_hours=20)
        # Also set send_pause as run-level stop (will be auto-cleared by 08:30 inbox stage)
        set_config('send_pause', 'true')
        set_config('pause_reason', '; '.join(risk_reasons))
        print(f"  [RISK] Run stop + temporary_risk_block (cooldown 20h): {'; '.join(risk_reasons)}")
        print(f"  [RISK] Scheduler NOT paused. Auto-recovery at next 08:30 risk review.")
    elif run_stop:
        print(f"  [RISK-DRY] Would trigger run_stop + temporary_risk_block: {'; '.join(risk_reasons)}")
    ...
    return {
        ...
        'paused': run_stop, 'pause_reasons': risk_reasons,
        'risk_type': 'temporary_risk_block' if run_stop else 'none',
    }
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if monitor.get('pause_reasons'):
            for pr in monitor['pause_reasons']:
                f.write(f"- [RUN STOP] {pr}\n")
        else:
            f.write("- No risk triggers\n")
        f.write(f"- risk_type: {monitor.get('risk_type', 'none')}\n")
        f.write(f"- send_pause: {get_config('send_pause')}\n")
        gate = get_risk_gate()
        f.write(f"- risk_gate: {gate['status']} ({gate['reason']})\n")
        f.write(f"- cooldown_until: {gate['cooldown_until']}\n")
        f.write(f"- manual_pause: {get_manual_pause()}\n")
        f.write(f"- standing_authorization: {get_standing_authorization()}\n")
        f.write(f"- scheduler_enabled: {get_scheduler_enabled()}\n\n")
...
def main():
    ...
    parser.add_argument('--catch-up-only', action='store_true', help='Only send remaining gap, not full target')
    parser.add_argument('--execution-mode', default='daily_outreach', choices=[
        'status_query', 'inventory_recovery', 'shadow_run', 'daily_outreach', 'manual_live'
    ])
    ...
    # ============================================================
    # v3.0: Run Lock — prevent duplicate daily_outreach
    # ============================================================
    today = datetime.now().strftime('%Y-%m-%d')
    if args.execution_mode == 'daily_outreach' and not dry_run:
        locked, holder = check_run_lock(today)
        if locked:
            print(f"\n[LOCKED] daily_outreach already running for {today} (holder: {holder})")
            print("[LOCKED] Exiting to prevent duplicate sends.")
            return
        if not acquire_run_lock(today):
            print(f"\n[LOCKED] Failed to acquire run lock for {today}")
            return
        print(f"\n[LOCK] Acquired run lock for {today}")
    
    # ============================================================
    # v3.0: Set execution_mode for this run
    # ============================================================
    set_execution_mode(args.execution_mode)
    
    # ============================================================
    # v3.0: Catch-up mode — only send remaining gap
    # ============================================================
    if args.catch_up_only:
        actual_sent = get_today_sent_asia_shanghai()
        gap = max(0, get_daily_target() - actual_sent)
        if gap == 0:
            print(f"\n[CATCH-UP] Daily target already met ({actual_sent}/{get_daily_target()}). Nothing to do.")
            release_run_lock(today)
            return
        print(f"\n[CATCH-UP] Today sent: {actual_sent}, Target: {get_daily_target()}, Gap: {gap}")
        target_count = gap
        print(f"[CATCH-UP] Adjusted target to gap: {target_count}")
    
    # ============================================================
    # v3.0: 08:30 Inbox Stage — auto risk recovery
    # ============================================================
    if args.execution_mode == 'daily_outreach':
        inbox_result = step_inbox_risk_recovery()
        if inbox_result.get('blocking'):
            print("[BLOCKED] Manual pause active. Cannot proceed with daily_outreach.")
            release_run_lock(today)
            return
    
    # Check if we're authorized to send
    if args.execution_mode == 'daily_outreach' and not dry_run:
        can_send, block_reason = can_send_live()
        if not can_send:
            print(f"\n[BLOCKED] Cannot send: {block_reason}")
            print(f"[INFO] Scheduler NOT paused. Check risk_gate/manual_pause.")
            release_run_lock(today)
            return

    print("=" * 60)
    print("DAILY OPERATOR AUTO v3.0")
    print("=" * 60)
    print(f"Execution mode: {args.execution_mode}")
    print(f"Send mode: {'DRY RUN' if dry_run else 'LIVE'}")
    ...
    print(f"Catch-up: {'ENABLED' if args.catch_up_only else 'DISABLED'}")
    print(f"standing_authorization: {get_standing_authorization()}")
    print(f"manual_pause: {get_manual_pause()}")
    risk_gate = get_risk_gate()
    print(f"risk_gate: {risk_gate['status']} ({risk_gate['reason']})")
    ...
    # Deduct today_sent: only send remaining quota
    today_sent = get_today_sent_count()
    remaining = max(0, target_count - today_sent)
    if today_sent > 0:
        print(f"\n  Today sent already: {today_sent}/{target_count}")
        print(f"  Remaining to send:  {remaining}")
    if len(planned) > remaining:
        print(f"  [QUOTA] Trimming plan: {len(planned)} → {remaining} (to stay within daily target)")
    ...
    # Final summary — v3.0: separate current run vs daily business
    print(f"\n{'='*60}")
    ...
    print(f"    send_enabled:    {not dry_run}")
    print(f"    run_status:      {'completed' if not monitor.get('paused') else 'run_stop'}")
    print(f"    risk_type:       {monitor.get('risk_type', 'none')}")
    
    actual_sent_today = get_today_sent_asia_shanghai()
    daily_target = get_daily_target()
    daily_gap = get_daily_gap()
    
    print(f"\n  Daily Business:")
    print(f"    today_sent:      {actual_sent_today}/{daily_target}")
    print(f"    daily_gap:       {daily_gap}")
    print(f"    daily_status:    {'completed' if daily_gap == 0 else 'underfilled'}")
    
    gate = get_risk_gate()
    print(f"\n  Scheduler:")
    print(f"    scheduler:       {'ACTIVE' if get_scheduler_enabled() else 'PAUSED'}")
    print(f"    manual_pause:    {get_manual_pause()}")
    print(f"    standing_auth:   {get_standing_authorization()}")
    print(f"    risk_gate:       {gate['status']} ({gate['reason']})")
    print(f"    cooldown:        {gate['cooldown_until']}")
    print(f"    next_run:        tomorrow 09:00 Asia/Shanghai")
    
    # Release run lock
    if args.execution_mode == 'daily_outreach':
        release_run_lock(today)
        print(f"\n  [LOCK] Released run lock for {today}")
