"""BD Production Final Preflight — 2026-08-21 (read-only gate, NO SMTP)."""
...
diag_plain_planned = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-21' AND status='planned'")
diag_plain_sent = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-21' AND status='sent'")
diag_plain_cancelled = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-21' AND status='cancelled'")
diag_plain_auth = conn.execute(
    "SELECT authorization_id, status FROM send_authorizations WHERE outreach_batch_date='2026-08-21' ORDER BY id DESC LIMIT 1"
).fetchone()
tonight_pending_exists = os.path.exists("tonight_pending_20260821.json")
...
result = {
    ...
    "diagnostics": {
        "plain_date_2026_08_21": {
            ...
            "note": "Today's prepared leads written under outreach_batch_date='2026-08-21' (plain date)?",
        },
        "tonight_pending_20260821_json_exists": tonight_pending_exists,
