"""BD Production Final Preflight — 2026-08-20 (read-only gate, NO SMTP)."""
...
step9 = {
    ...
    "poller_running": ps.get("running"),
    "poller_current_errors": ps.get("current_errors"),
...
# Diagnostics: today's activity under wrong batch id (plain date)
diag_plain_planned = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-20' AND status='planned'")
diag_plain_sent = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-20' AND status='sent'")
diag_plain_cancelled = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-20' AND status='cancelled'")
diag_plain_auth = conn.execute(
    "SELECT authorization_id, status FROM send_authorizations WHERE outreach_batch_date='2026-08-20' ORDER BY id DESC LIMIT 1"
).fetchone()
tonight_pending_exists = os.path.exists("tonight_pending_20260820.json")

# stale leftover auths (from prior days) — expand step10 detail
stale_auths_all = conn.execute(
    "SELECT authorization_id, outreach_batch_date, status, expires_at FROM send_authorizations "
    "WHERE status IN ('approved','active','pending') ORDER BY id DESC"
).fetchall()
...
result = {
    ...
    "diagnostics": {
        "plain_date_2026_08_20": {
            "planned": diag_plain_planned,
            "sent": diag_plain_sent,
            "cancelled": diag_plain_cancelled,
            "latest_auth": dict(diag_plain_auth) if diag_plain_auth else None,
            "note": "Today's prepared leads written under outreach_batch_date='2026-08-20' (plain date)?",
        },
        "tonight_pending_20260820_json_exists": tonight_pending_exists,
        "all_active_auths": [dict(r) for r in stale_auths_all],
    },
