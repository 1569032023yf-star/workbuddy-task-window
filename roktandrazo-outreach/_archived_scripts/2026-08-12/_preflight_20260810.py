"""BD Production Final Preflight — Read-only gate verification for 2026-08-10 ET 10:00."""
import sqlite3, json, os, hashlib, sys, urllib.request, urllib.error
...
if auth_row:
    results['gates']['step3_auth_exists'] = {
        'pass': True,
        'authorization_id': auth_row[0],
        'plan_id': auth_row[1],
        'approved_entry_count': auth_row[2],
        'preflight_status': auth_row[3],
        'approved_at': auth_row[4],
        'expires_at': auth_row[5],
        'status': auth_row[6]
    }
    approved_count = auth_row[2] or 0
    # Auth entries detail
    c.execute('SELECT COUNT(*) FROM send_authorization_entries WHERE authorization_id = ?', (auth_row[0],))
...
# === STEP 7: Template SHA ===
# Load bd_template module and read _TEMPLATE_SHA256
...
# Uses sync_0845_last_success_at as proxy for poller health
...
if hb_row:
    ...
    try:
        ...
        results['gates']['step9_poller_heartbeat'] = {
            'pass': stale_min < 120,  # within 2 hours = healthy
...
# Only count "active" old plans: status='planned' (not sent/skipped)
...
# Active = not consumed/revoked/expired
...
snap_path = 'data/frozen_new_outreach_20260810_et1000.json'
results['frozen_snapshot_exists'] = os.path.exists(snap_path)

# A0 pool size
c.execute("SELECT COUNT(*) FROM leads WHERE status = 'new' AND confidence_score = 'A' AND email_verified_on_official_site = 1 AND email IS NOT NULL")
...
c.execute("INSERT OR REPLACE INTO system_config (key, value, updated_at) VALUES ('SMTP_enabled', '0', datetime('now'))",)
if blockers:
