...
# ── P0 Enhanced Gate (read-only) ─────────────────────────────
# 在现有检查之后追加 preflight_gate.run_preflight 结果，合并进输出。
# 只读：persist_cache=False，不写 DB。
print("\n── P0 Enhanced Gate (preflight_gate.run_preflight) ──")
try:
    sys.path.insert(0, str(PROJECT_DIR))
    import preflight_gate
    # 优先取当前最近存在 planned 计划的批次；没有则退回当日 batch_date
    gate_batch = batch_date
    cand = conn.execute(
        "SELECT MAX(outreach_batch_date) m FROM final_send_plan WHERE status='planned'"
    ).fetchone()
    if cand and cand["m"]:
        gate_batch = cand["m"]
    gate_result = preflight_gate.run_preflight(
        conn, gate_batch, require_dns=True, persist_cache=False
    )
    print(f"  [GATE] batch={gate_result['batch_id']} plan_count={gate_result['plan_count']}")
    for c in gate_result["checks"]:
        print(f"  [GATE] {c['status'].upper():4s} {c['name']}: {c['detail'][:120]}")
    if gate_result["blocks"]:
        print(f"  [GATE] BLOCKED ({len(gate_result['blocks'])}): {gate_result['blocks']}")
    else:
        print("  [GATE] ALL PASS")
except Exception as gate_err:
    gate_result = {"pass": False, "error": str(gate_err)[:200],
                   "batch_id": batch_date, "checks": [], "blocks": ["gate_error"],
                   "smtp_blocked": True}
    print(f"  [GATE] run_preflight failed: {gate_err}")
...
# 合并 P0 Enhanced Gate 结果（保持原字段不动，新增 gate 字段）
result["gate"] = gate_result
if gate_result.get("smtp_blocked"):
    print(f"\n  [GATE] SMTP blocked=1 — 即使原检查全过，gate 仍阻断发信")
