"""BD Production Final Preflight — 2026-08-19 (read-only gate, NO SMTP)."""
...
diag_plain_planned = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-19' AND status='planned'")
diag_plain_sent = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-19' AND status='sent'")
diag_plain_cancelled = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-19' AND status='cancelled'")
diag_plain_auth = conn.execute(
    "SELECT authorization_id, status FROM send_authorizations WHERE outreach_batch_date='2026-08-19' ORDER BY id DESC LIMIT 1"
).fetchone()
tonight_pending_exists = os.path.exists("tonight_pending_20260819.json")
...
result = {
    ...
    "diagnostics": {
        "plain_date_2026_08_19": {
            ...
            "note": "Today's prepared leads were written under outreach_batch_date='2026-08-19' (plain date), NOT the canonical batch id.",
        },
        "tonight_pending_20260819_json_exists": tonight_pending_exists,
    },
