...
def build_context(conn) -> dict[str, Any]:
    ...
    hard_bounced = {r[0].strip().lower() for r in c.execute(
        "SELECT email FROM bounce_log WHERE bounce_type = 'hard' AND email IS NOT NULL"
    ).fetchall()}

    policy_bounced = {r[0].strip().lower() for r in c.execute(
        "SELECT email FROM bounce_log WHERE bounce_type = 'policy' AND email IS NOT NULL"
    ).fetchall()}
    ...
    return {
        ...
        "policy_bounced_emails": policy_bounced,
        "sent_emails": sent,
