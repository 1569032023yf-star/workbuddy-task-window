"""P1.7 批次1 v2：Machine-First 官网邮箱自动扫描。
每家：首页 + contact/wholesale 页（最多 3 页）。只记录官网第一方页面实际出现的完整邮箱。
"""
...
from website_verifier import (verify_website, extract_emails_from_html,
                              ...
                              find_wholesale_page, extract_contact_form_from_html)

clues = json.load(open('output/p17_batch1_dir_clues.json'))
batch = json.load(open('output/p17_batch1.json'))
...
for s in batch:
    ...
    try:
        ...
        cp = find_contact_page(links)
        wp = find_wholesale_page(links)
        # 抓 contact/wholesale（最多再 2 页）
        for p in (cp, wp):
    ...
    if all_emails and domain_match:
        ...
        rec['status'] = 'EMAIL_FOUND_OTHER_DOMAIN'  # 需人工确认归属
    else:
        rec['status'] = 'NO_EMAIL'
    out.append(rec)
    print(f"{name:<36} {rec['status']:<22} domain={domain:<32} emails={all_emails[:3]}")
    time.sleep(0.2)

json.dump(out, open('output/p17_batch1_scan2.json', 'w'), ensure_ascii=False, indent=2)
print('\n=== 汇总 ===')
