"""BD Production Final Preflight — 2026-08-18 (read-only gate, NO SMTP)."""
...
diag_plain_planned = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-18' AND status='planned'")
diag_plain_sent = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-18' AND status='sent'")
diag_plain_cancelled = q1("SELECT COUNT(*) c FROM final_send_plan WHERE outreach_batch_date='2026-08-18' AND status='cancelled'")
diag_plain_auth = conn.execute(
    "SELECT authorization_id, status FROM send_authorizations WHERE outreach_batch_date='2026-08-18' ORDER BY id DESC LIMIT 1"
).fetchone()
tonight_pending_exists = os.path.exists("tonight_pending_20260818.json")
...
result = {
    ...
    "diagnostics": {
        "plain_date_2026_08_18": {
            ...
            "note": "Today's canary + prepared leads were written under outreach_batch_date='2026-08-18' (plain date), NOT the canonical batch id.",
        },
        "tonight_pending_20260818_json_exists": tonight_pending_exists,
    },
